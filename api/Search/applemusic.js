...
export default {
  ...
  desc: "Scraper pencarian lagu/artis Apple Music",
  category: "Search",
  path: "/search/applemusic?query=",
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const q = process.argv.slice(2).join(" ") || "consume";

  console.log(`🔍 Mencari "${q}" ...`);
  const out = await applemusicSearch(q);
