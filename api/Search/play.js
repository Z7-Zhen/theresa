...
class Youtubers {
  ...
  async key() {
    ...
    return await subtle.importKey("raw", raw, { name: "AES-CBC" }, false, ["decrypt"]);
  }
  ...
  async Data(base64Terenkripsi) {
    ...
    const subtle = crypto.webcrypto.subtle;
    const hasil = await subtle.decrypt({ name: "AES-CBC", iv }, kunci, data);
...
export default [
  {
    name: "Play",
    desc: "Cari dan putar lagu YouTube dari query (audio only)",
    category: "Search",
    path: "/search/play?apikey=&q=",
    async run(req, res) {
      try {
        ...
        const results = await yt.downloadyt(video.url, "360", "audio");
